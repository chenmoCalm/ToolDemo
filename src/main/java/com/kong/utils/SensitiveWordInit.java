...
//		this.getClass().getResourceAsStream("/excleTemplate/test.xlsx");
		Resource resource = new ClassPathResource("CensorWords.txt");//文件放在resource目录下
		InputStream is = resource.getInputStream();
		InputStreamReader read = new InputStreamReader(is,ENCODING);
		try (InputStream is1 = resource.getInputStream();){
			if(resource.exists()){      //文件流是否存在
